...
pass are **complete and user-accepted**. The web app is done. **Task 7 (Expo
foundation and offline data layer) is now the active task** — the user approved
starting it.
...
5. **Task 7 is now in progress.** It is the first Expo/mobile task. Mirror the
   web implementations of the shared contracts rather than reinventing them:

   - Shared domain: `packages/domain/src/` (already consumed by `mobile/` via
     `@household-hub/domain`).
   - Durable operation queue reference: `src/lib/operations/` (web/IndexedDB).
     The native queue is the same command contract on SQLite.
   - Supabase client reference: `src/lib/supabase.ts`.

   Native environment variables are `EXPO_PUBLIC_SUPABASE_URL` /
   `EXPO_PUBLIC_SUPABASE_ANON_KEY` (see `mobile/.env.example`); native scheme is
   `householdhub://`, app id `com.conlegs.householdhub`, portrait phones only
   (already set in `mobile/app.json` + `mobile/eas.json`). Use TDD for behavior
   changes and update `progress.md` at **every sub-checkpoint** (HEAD, commit,
   verification, resume point) — per the user's directive.
...
| Web parity correction pass (1–6) | Complete & accepted | Final handoff at `071b43b` |
| **7. Expo foundation and offline data layer** | **In progress** | **Active task** |
...
## Task 7 — Expo foundation and offline data layer (in progress)
...
Sub-checkpoints will be recorded here as they complete.
