 ...
 * optimistically and to replay it after a cold start.
 *
 * The row IS the command — nothing about a queued write lives only in memory,
 * so a relaunch mid-flight resumes exactly where it left off. Mirrors the web
 * client's `QueuedOperation` so both platforms share one durable contract.
 */
 ...
 * happened. A losing mutation is discarded rather than merged or re-edited.
 */
