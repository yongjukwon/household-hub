import { queryKeys, type ReminderPreset } from '@household-hub/domain'
...
/**
 * All calendar events for the household with their reminder presets. Mirrors
 * the web client's `useCalendarEvents` exactly: the client expands recurrence
 * and multiday spans per visible month (see `events.ts`), so a single
 * household-scoped read backs every month view.
 */
