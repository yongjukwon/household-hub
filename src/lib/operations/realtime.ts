 ...
 * household-scoped subscription covers all five features instead of one
 * channel per table. An event invalidates the household's queries; the refetch
 * then goes back through the optimistic overlay, so an incoming change can
 * never erase a command this device has not sent yet.
 *
 * A change also means the connection is alive, which is a good moment to
 * retry anything the queue is holding.
 *
